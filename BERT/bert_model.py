...
class MyModel(nn.Module):
    def __init__(self):
        ...
        # # 添加一个线性层用于降维
        # self.downsample_layer = nn.Linear(self.args.num_filters, 32)

        # 读取文件并计算行数
        with open(parsers().new_vocab, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            line_count = len(lines)
        tokenizer = BertTokenizer.from_pretrained(self.args.bert_pred,  clean_up_tokenization_spaces=True)
        new_len = len(tokenizer)+line_count
        print("1我的长度---", line_count, " 原长度---", len(tokenizer), " 新长度---", new_len)
    ...
    def forward(self, x):
        ...
        cls_vector = hidden_out.last_hidden_state[:, 0, :]
        # # 对 CLS 向量进行降维
        # downsampled_cls_vector = self.downsample_layer(cls_vector)
        # # 返回预测结果
        # return pred, downsampled_cls_vector
        return pred, cls_vector
